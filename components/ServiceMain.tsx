            ...
            <Card.Body>
                <Accordion>
                    {partnerServices.map((service, index) => (
                        <Accordion.Item eventKey={String(index)} key={service.name}>
                            <Accordion.Header><img src={CURRENCIES.find(c => c.category === service.category)?.iconPath} width={24} height={24} alt={String(index)} />{service.name} </Accordion.Header>
                            <Accordion.Body>
